...
export async function getExamDataSistemed2(examId: number) {
  const response = await axios.get(`/sistemed2_api/get_exam_data/${examId}`, { withCredentials: true });

  return response;
}

export async function acceptExamSistemed2(examId: number, urgency = false) {
...
export async function rejectExamSistemed2(
  examId: number,
...
export async function postMarkersSistemed2(examId: number, points: FiducialPoints) {
  const requestBody = {
    p_start: points.p,
    qrs_start: points.q,
    r: points.r,
...
export async function postDiagnosticsSistemed2(examId: number, diagnostics: DiagnosticSistemed2[]) {
